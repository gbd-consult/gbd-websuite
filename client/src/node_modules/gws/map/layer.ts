...
export class FeatureLayer extends OlBackedLayer<ol.layer.Vector> implements types.IFeatureLayer {
    ...
    beforeDraw() {
        ...
        if (this.loadingStrategy === 'bbox') {
            ...
            // let src = this.createSource();
            // for (let f of Object.values(this.fMap))
            //     if (f.oFeature)
            //         src.addFeature(f.oFeature);
            //
            // this._oLayer.setSource(src);
        }
    ...
    addFeatures(features) {
        if (this.loadingStrategy === 'single') {
            this.clear();
    ...
    clearUnchanged() {
        let oldMap = this.fMap;
        let newMap: { [uid: string]: types.IFeature } = {};

        for (let fe of Object.values(oldMap)) {
            if (fe.isDirty || fe.isNew) {
                newMap[fe.uid] = fe;
            }
        }
...
/*
    ...
    __beforeDraw() {
        if (this.loadState === 'all')
            return;

        if (this.loadState === 'loading') {
            // console.log('Vector:load', this.uid, 'ALREADY LOADING');
            return;
        }

        if (this.props.loadingStrategy === 'bbox') {
            let bbox = this.map.bbox;
            if (String(this.lastBbox) === String(bbox)) {
                return;
            }
            this.lastBbox = [...bbox];
            this.load(bbox);
    ...
    protected load(bbox?: ol.Extent) {
        ...
        console.log('Vector:load', this.uid, 'started', bbox);
        let req = this.map.app.server.queueLoad(this.uid, url, '');


        this.loadState = 'loading';

        req.then(res => this.loaded(res, bbox ? '' : 'all'));

    }

    protected loaded(res, lstate) {
        if (!res.features) {
            console.log('Vector:load', this.uid, 'ERROR', res);
            return;
        }

        let fc = this.map.newFeatureCollectionFromProps(res.features);

        console.log('Vector:load', this.uid, 'ok', fc.features.map(f => f.uid));

        let newMap = {};

        for (let f of Object.values(this.fMap)) {
            if (f.isDirty || f.isNew) {
                console.log('Vector:load', this.uid, 'keep dirty', f.uid);
                newMap[f.uid] = f;
                continue;
            }
            if (f.isFocused) {
                // @TODO update
                console.log('Vector:load', this.uid, 'keep focused', f.uid);
                newMap[f.uid] = f;
                continue;
            }
            this.olRemoveFeature(f);
