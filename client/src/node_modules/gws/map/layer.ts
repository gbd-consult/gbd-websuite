...
export class VectorLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    addFeatures(fs) {
        ...
        // });
        //
        // console.log('newFs', newFs.length, 'toAdd', toAdd.length, 'toRemove', toRemove.length);
        //
        // toRemove.forEach(f => this.source.removeFeature(f.oFeature));
        // this.source.addFeatures(toAdd.map(f => f.oFeature));
        //
        // this.features = newFs;
        // this.map.changed();
    ...
    protected async loader(extent, resolution, proj) {
        console.log('VECTOR_LOADER', 'start');
        let fs = await this.loadFeatures(extent, resolution);
        this.addFeatures(fs);
        console.log('VECTOR_LOADER', 'end');
    }
}
