...
function parseMarkApply(val) {
    val = val || 'point,line,polygon';
    val = val.split(',').map(x => x.trim());

    return {
        'Circle': false,
        'LineString': val.indexOf('line') >= 0,
        'MultiLineString': val.indexOf('line') >= 0,
        'MultiPoint': val.indexOf('point') >= 0,
        'Point': val.indexOf('point') >= 0,
        'MultiPolygon': val.indexOf('polygon') >= 0,
        'Polygon': val.indexOf('polygon') >= 0,
    };
}
...
function parseExtras(css) {
    ...
    opts['mark-apply'] = parseMarkApply(css['mark-apply']);

    return opts;
}

function markBounds(geom, markApply) {
    let t = geom.getType();
    ...
    switch (t) {
        ...
        case 'MultiPolygon':
            return geom.getPolygon(0).getLinearRing(0);
    }
...
export class CssStyle implements types.IMapStyle {
    ...
    apply(geom, label, resolution) {
        ...
        let bounds = markBounds(geom, this.extras['mark-apply']);
