...
abstract class OlBackedLayer<T extends ol.layer.Base> extends Layer {
    _oLayer: T;
...
abstract class BaseVectorLayer extends OlBackedLayer<ol.layer.Vector> {
    props: api.VectorLayerProps;

    style: types.IMapStyle = null;
    editStyle: types.IMapStyle = null;
...
export class FeatureLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    get printItem() {
        return {
            features: this.features.map(f => f.printProps),
...
export class VectorLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    addFeatures(fs) {
        let uids = this.features.map(f => f.uid);
        let cnt = 0;

        fs.forEach(f => {
            if (uids.indexOf(f.uid) < 0) {
    ...
    async loadFeatures(where): Promise<Array<types.IMapFeature>> {
        let params = {
            layerUid: this.uid,
            bbox: null
        };
