...
export class Feature implements types.IMapFeature {
    ...
    constructor(map, args: types.IMapFeatureArgs) {
        ...
        this.styleNames = this.map.style.getMap({
    ...
    get currentStyleName() {
        return this.styleNames[this.mode];
    }
    ...
    protected updateOlStyle() {
        if (!this.oFeature || !this.currentStyleName)
            return;

        this.oFeature.setStyle((oFeature: ol.Feature, resolution: number) => {
            let style = this.map.style.at(this.currentStyleName);
            if (style) {
