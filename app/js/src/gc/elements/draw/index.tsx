...
let _master = (cc: gc.types.IController) => cc.app.controller(MASTER) as DrawController;

interface DrawProps extends gc.types.ViewProps {
    controller: DrawController;
    title: string;
    drawCurrentShape: string;
    drawEnabledShapes?: Array<string>;
    drawStyle?: gc.types.StyleArg;
    drawMode: boolean;
}
...
class DrawToolboxView extends gc.View<DrawProps> {
    render() {
        ...
        let button = (type, cls, tooltip) => {
            return <Cell>
                <gc.ui.Button
                    {...gc.lib.cls(cls, type === shapeType && 'isActive')}
                    tooltip={tooltip}
...
export class DrawController extends gc.Controller {
    uid = MASTER;
