...
class LoaderView extends gws.View<LoaderProps> {
    render() {
        ...
        return <div {...gws.lib.cls('modInfobarWidget', 'modInfobarLoader', rc && 'isActive')}>
            <Row>
                <Cell>
                    <gws.ui.Button className='modInfobarLoaderIcon'/>
                </Cell>
                {rc > LOADER_PROGRESS_MIN && <Cell width={bars * 2}>
                    {gws.lib.range(bars).map(n => <div className='modInfobarLoaderBar' key={n}/>)}
                </Cell>}
            </Row>
        </div>
    }
...
class ScaleView extends gws.View<ScaleProps> {
    render() {
        ...
        let res = this.props.controller.map.resolutions,
            max = Math.max(...res.map(gws.lib.res2scale)),
            min = Math.min(...res.map(gws.lib.res2scale));
        ...
        return <div className="modInfobarWidget modInfobarScale">
            ...
            <Cell className="modInfobarScaleSlider">
                <gws.ui.Slider
                    minValue={min}
                    maxValue={max}
                    value={gws.lib.asNumber(this.props.mapEditScale)}
...
class RotationView extends gws.View<RotationProps> {
    ...
    render() {
        let cc = this.props.controller;
        let n = gws.lib.asNumber(this.props.mapEditAngle);
...
class LinkView extends gws.View<LinkProps> {
    render() {
        return <div {...gws.lib.cls('modInfobarWidget', 'modInfobarLink', this.props.className)}>
            <a
...
class AboutDialog extends gws.View<AboutViewProps> {
    render() {
        ...
        let content = <div className="modAboutDialogContent">
            ...
            <div className="p4">&copy; Geoinformatikbüro Dassau GmbH 2006–2023</div>
            <div className="p2">
