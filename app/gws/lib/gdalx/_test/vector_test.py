...
def test_shp(tmp_path):
    ...
    with gdalx.open_vector(f'{tmp_path}/shape.shp', 'w') as ds:
        ...
        la.insert(recs_a)
    dbf = open(f'{tmp_path}/shape.dbf', 'rb').read()
    assert recs_a[0].attributes['c_str'].encode('utf8') in dbf

    with gdalx.open_vector(f'{tmp_path}/shape.shp', 'r') as ds:
        la = ds.layer(0)
...
def test_shp_with_encoding(tmp_path):
    cols = dict(
    ...
    with gdalx.open_vector(f'{tmp_path}/shape_iso.shp', 'w', encoding='ISO-8859-1') as ds:
        la = ds.create_layer('', cols)
        la.insert(recs_a)

    dbf = open(f'{tmp_path}/shape_iso.dbf', 'rb').read()
    assert names[0].encode('ISO-8859-1') in dbf

    # NB: "encoding" should only be passed when no "cpg" file is present
    osx.unlink(f'{tmp_path}/shape_iso.cpg')

    with gdalx.open_vector(f'{tmp_path}/shape_iso.shp', 'r', encoding='ISO-8859-1') as ds:
        la = ds.layer(0)
        assert [r.attributes for r in recs_a] == [r.attributes for r in la.get_all()]

    with gdalx.open_vector(f'{tmp_path}/shape_utf.shp', 'w', encoding='utf8') as ds:
        la = ds.create_layer('', cols)
        la.insert(recs_a)

    dbf = open(f'{tmp_path}/shape_utf.dbf', 'rb').read()
    assert names[0].encode('utf8') in dbf

    # NB: "encoding" should only be passed when no "cpg" file is present
    osx.unlink(f'{tmp_path}/shape_utf.cpg')

    with gdalx.open_vector(f'{tmp_path}/shape_utf.shp', 'r', encoding='UTF8') as ds:
        la = ds.layer(0)
        assert [r.attributes for r in recs_a] == [r.attributes for r in la.get_all()]


def test_gpkg(tmp_path):
    cols = dict(
        c_date=gws.AttributeType.date,
        c_float=gws.AttributeType.float,
        c_int=gws.AttributeType.int,
...
def test_gpkg_datetime(tmp_path):
    crs = gws.lib.crs.require(25833)
...
def test_geojson_time(tmp_path):
    ...
    with gdalx.open_vector(f'{tmp_path}/time.gpkg', 'r') as ds:
        recs_b = ds.layer(0).get_all()
...
def test_geojson_floatlist(tmp_path):
...
def test_geojson_intlist(tmp_path):
...
def test_geojson_strlist(tmp_path):
...
def test_gpkg_multiple_layers(tmp_path):
    crs = gws.lib.crs.require(25833)
    cols = dict(name=gws.AttributeType.str)

    path = f'{tmp_path}/multi.gpkg'
    with gdalx.open_vector(path, 'w') as ds:
...
def test_gpkg_geometry_types(tmp_path):
    crs = gws.lib.crs.require(25833)
    cols = dict(name=gws.AttributeType.str)
    ...
    path = f'{tmp_path}/geomtypes.gpkg'
    with gdalx.open_vector(path, 'w') as ds:
...
def test_gpkg_get_by_fid(tmp_path):
    crs = gws.lib.crs.require(25833)
    cols = dict(name=gws.AttributeType.str)
...
def test_shp_bool(tmp_path):
    ...
    with gdalx.open_vector(f'{tmp_path}/bool.shp', 'r') as ds:
        la = ds.layer(0)
        desc = la.describe()
