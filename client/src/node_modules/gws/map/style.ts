...
export class CssStyle implements types.IMapStyle {
    ...
    constructor(map, css) {
        ...
        this.css = Object.assign(
            {},
            _CSS_DEFAULTS,
            stripPrefix(typeof css === 'string' ? toDict(css) : css))

        this.markImage = parseMark(this.css);
        this.labelOpts = parseLabel(this.css);

        let fill = parseFill(this.css, ''),
            stroke = parseStroke(this.css, '');

        if (fill || stroke) {
            this.shapeStyle = new ol.style.Style(compact({fill, stroke}));

            let pointSize = pxValue(this.css['point-size']);
        ...
        this.labelPlacement = this.css['label-placement'];
        this.labelMaxResolution = numValue(this.css['label-max-resolution']);
        this.labelMinResolution = numValue(this.css['label-min-resolution']);
    }
