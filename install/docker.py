...
Options:
    ...
    -qgis <qgis-version>
        ...
        The builder looks for QGIS server images on 'gws-files.gbd-consult.de', 
        see the subdirectory './qgis' on how to create a QGIS image.      
    ...
    [-appdir <dir>]
        app directory to copy to "/gws-data" in the image.
...
class Builder:
    ...
    def dockerfile(self):
        ...
        if self.image_kind == 'qgis':
            ...
            apts = set(self.qgis_apts + self.gws_apts)
            pips = set(self.qgis_pips + self.gws_pips)
        else:
            apts = self.gws_apts
            pips = self.gws_pips
    ...
    def prepare(self):
        ...
        if self.qgis_version:
            run(self.f("curl -sL '{qgis_url}' -o {qgis_dir}.tar.gz && tar -xzf {qgis_dir}.tar.gz && mv qgis-for-gws {qgis_dir}"))
            run(self.f("curl -sL '{alkisplugin_url}' -o {alkisplugin_dir}.tar.gz && tar -xzf {alkisplugin_dir}.tar.gz"))
