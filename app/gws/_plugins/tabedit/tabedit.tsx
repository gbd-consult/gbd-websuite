...
class TabeditDialog extends gws.View<TabeditViewProps> {
    ...
    componentDidMount() {
        ...
        if (this.focusRef.current)
            gws.lib.nextTick(() => this.focusRef.current.focus());
    }
    ...
    applyFilters(records) {
        ...
        for (let [ncol, val] of gws.lib.entries(filter)) {
            records = records.filter(rec => !val ||
                (gws.lib.empty(rec[ncol]) ? '' : String(rec[ncol]).toLowerCase()).indexOf(val.toLowerCase()) >= 0
            );
        }
    ...
    tableDialog() {
        ...
        let footer = <Row>
            ...
            <Cell>
                <gws.ui.Button
                    ...
                    disabled={gws.lib.empty(dirtyFields)}
        ...
        return <gws.ui.Dialog
            {...gws.lib.cls('modTabeditDialog', this.props.appDialogZoomed && 'isZoomed')}
...
class TabeditSidebarView extends gws.View<TabeditViewProps> {
    render() {
        ...
        return <sidebar.Tab>
            ...
            <sidebar.TabBody>
                {
                    this.props.tabeditTables.map(t => <Row key={gws.uid}>
                        <Cell>
                        ...
                        <Cell>
                            <gws.ui.Touchable
                                ...
                                whenTouched={() => cc.startEdit(gws.uid)}
                            >{gws.title}</gws.ui.Touchable>
                        </Cell>
                    </Row>)
                }
...
class TabeditController extends gws.Controller {
    ...
    addRecord() {
        ...
        rec['_uid'] = gws.lib.uniqId('tabedit');
    ...
    async saveData() {
        ...
        let updRecs = {};
        for (let [key, val] of gws.lib.entries(dirtyFields)) {
