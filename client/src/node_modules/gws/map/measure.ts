...
export const SPHERE = 2;
export const ELLIPSOID = 3;
...
function geodesic(projection) {
    ...
    if (ellpsCache[ellps])
        return ellpsCache[ellps];

    if (!Ellipsoids[ellps])
        return ellpsCache['WGS84'];

    ellpsCache[ellps] = new GeographicLib.Geodesic.Geodesic(Ellipsoids[ellps][0], 1 / Ellipsoids[ellps][2]);
    console.log('init ellispoid ', ellps, Ellipsoids[ellps]);
    return ellpsCache[ellps];
}

export function lengthCoord(coords: Array<ol.Coordinate>, projection: ol.proj.Projection, mode) {
...
export function length(geom: ol.geom.Geometry, projection: ol.proj.Projection, mode) {

    if (mode === CARTESIAN || mode === ELLIPSOID) {
...
export function areaCoord(coords: Array<ol.Coordinate>, projection: ol.proj.Projection, mode) {
...
export function area(geom: ol.geom.Geometry, projection, mode) {
    if (mode === CARTESIAN) {
        if (geom.getType() !== 'Polygon')
            return 0;
...
export function distance(p1: ol.Coordinate, p2: ol.Coordinate, projection: ol.proj.Projection, mode) {
