...
def test_authenticate_with_bad_login_fails(root: gws.Root):
    ...
    usr = am.authenticate(am.methods[0], gws.Data(username='BAD', password='foo'), u.auth.requester(root))
    assert usr is None
...
def test_authenticate_with_wrong_method_fails(root: gws.Root):
    am = root.app.authMgr
    u.auth.add_user('me', 'foo')
    usr = am.authenticate(am.methods[1], gws.Data(username='me', password='foo'), u.auth.requester(root))
    assert usr is None
