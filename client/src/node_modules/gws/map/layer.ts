...
export class VectorLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    async loadFeatures(extent, resolution) {
        console.log('LOAD_FEATURES', this.uid, extent, resolution)
        ...
        if (res.error) {
            console.log('LOAD_FEATURES', this.uid, 'failed');
            return [];
        }

        console.log('LOAD_FEATURES', this.uid, res.features.length);
    ...
    protected async loader(extent, resolution, proj) {
        console.time('VECTOR_LOADER ' + this.uid);
        let fs = await this.loadFeatures(extent, resolution);
        this.addFeatures(fs);
        console.timeEnd('VECTOR_LOADER ' + this.uid);
    }
}
