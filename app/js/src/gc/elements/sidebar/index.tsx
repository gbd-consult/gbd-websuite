...
interface SidebarProps extends gc.types.ViewProps {
...
interface ButtonProps extends SidebarProps {
    item: gc.types.ISidebarItem;
}

class CloseButton extends gc.View<SidebarProps> {
    render() {
        return <gc.ui.Button
            {...gc.lib.cls('modSidebarCloseButton')}
...
class OpenButton extends gc.View<SidebarProps> {
    render() {
        return <gc.ui.Button
            {...gc.lib.cls('modSidebarOpenButton')}
...
class HeaderButton extends gc.View<ButtonProps> {
    render() {
...
class PopupHeaderButton extends gc.View<ButtonProps> {
    render() {
        let item = this.props.item,
            type = item.tag,
            active = type === this.props.sidebarActiveTab,
...
class Header extends gc.View<SidebarProps> {
    render() {
        ...
        return <div className="modSidebarHeader">
            <Row>
                ...
                {rest.length > 0 && <gc.ui.Button
                    {...gc.lib.cls('modSidebarOverflowButton', expanded && 'isActive')}
...
class SidebarOverflowView extends gc.View<SidebarProps> {
...
class SidebarView extends gc.View<SidebarProps> {
    render() {
        return <React.Fragment>
            {!this.props.sidebarVisible && <OpenButton {...this.props}/>}
            <div {...gc.lib.cls('modSidebar', this.props.sidebarVisible && 'isVisible')}
                style={{ width: this.props.controller.width }}
            >
                <Header {...this.props} />
                <div {...gc.lib.cls('modSidebarLeftContainer')}>
                    <Body {...this.props} />
...
class SidebarResizeHandle extends gc.View<SidebarProps> {
    render() {
        return <div {...gc.lib.cls('modSidebarResizeHandle', this.props.resizing && 'isResizing')}
...
class SidebarController extends gc.Controller {
    async init() {
        await super.init();
...
class _TabItem extends React.PureComponent<WithClassname> {
    ...
    render() {
        let cls = gc.lib.cls(this.className, this.props.className);
