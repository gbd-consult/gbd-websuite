...
def from_geojson(js, crs, key_column='id') -> gws.IFeature:
    ...
    return Feature(
        uid=uid,
        attributes=atts,
        shape=gws.base.shape.from_geometry(js['geometry'], crs),
    )
...
class Feature(gws.Object, gws.IFeature):
    ...
    # templates: t.Optional[gws.ITemplateCollection]
    # uid: str
    ...
    def __init__(self, uid, attributes=None, category=None, elements=None, shape=None, style=None):
        ...
        if shape:
            if isinstance(shape, gws.base.shape.Shape):
                self.shape = shape
            else:
                self.shape = gws.base.shape.from_props(shape)
