...
export class MapManager implements types.IMapManager {
    ...
    setViewExtent(extent, animate, padding = 0, minScale = 0) {
        ...
        if (minScale) {
            minRes = Math.min(
                ...
                tools.scale2res(minScale));
        }

        this.setViewState({
            center: ol.extent.getCenter(extent),
            resolution: tools.clamp(res, minRes, maxRes)
        }, animate);
    }
    ...
    readFeature(props: api.FeatureProps): types.IFeature {
        return this.featureFromProps(props);

    }

    readFeatures(propsList: Array<api.FeatureProps>): Array<types.IFeature> {
        return this.featureListFromProps(propsList);
    }

    featureListFromProps(propsList: Array<api.FeatureProps>): Array<types.IFeature> {
    ...
    featureFromProps(props: api.FeatureProps): types.IFeature {
        ...
        if (model) {
            for (let f of model.fields) {
                ...
                if (val && f.dataType === 'feature') {
                    atts[f.name] = this.featureFromProps(val);
                }

                if (val && f.dataType === 'featureList') {
    ...
    featureProps(feature: types.IFeature, depth?: number): api.FeatureProps {
        ...
        if (feature.cssSelector)
            style = this.style.getFromSelector(feature.cssSelector);
    ...
    loadModels(models: Array<api.ModelProps>) {
    ...
    protected async printItems(boxRect, dpi): Promise<Array<api.PrintItem>> {
        ...
        interface PrintItemOrBitmap {
            pi?: api.PrintItem
            bmpLayers?: Array<types.IMapLayer>
        }
        ...
        this.walk(this.root, la => {
            ...
            if (mode === 0 && la.isClient) {
                // cannot print `isClient` layers without a boxRect
                return;
            }

            if (pi) {
                let useBitmap = (mode === 1) || (mode === 2 && pi.type === 'raster') || (mode > 0 && la.isClient);
                if (useBitmap) {
