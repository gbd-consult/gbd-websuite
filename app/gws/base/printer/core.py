...
def start_job(root: gws.IRoot, req: gws.IWebRequest, params: types.Params) -> gws.lib.job.Job:
    job = _create(root, req, params)
    gws.server.spool.add(job)
    return job


def run_job(root: gws.IRoot, params: types.Params, project_uid: str, user: gws.IUser):
    w = _Worker(root, '', project_uid, params, user)
    return w.run()


def job_status(job: gws.lib.job.Job) -> types.StatusResponse:
...
class _Worker:
    def __init__(self, root: gws.IRoot, job_uid: str, project_uid: str, params: types.Params, user: gws.IUser):
