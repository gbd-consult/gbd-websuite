...
export class MapManager implements types.IMapManager {
    ...
    getStyle(p) {
        if (p instanceof CssStyle)
            return p;
        if (p.type === api.StyleType.css) {
            if (p.content)
                return CssStyle.fromSource(this, p.content);
            return CssStyle.fromSource(this, p.text);
        }
        if (p.type === api.StyleType.cssSelector) {
    ...
    protected bitmapItem(boxRect): api.PrintItem {
    ...
    async basicPrintParams(boxRect, dpi) {
        ...
        if (useBitmaps) {
            ...
            bitmapItem = await tools.delay(200, () => {
                console.time('creating_bitmap');
                let b = this.bitmapItem(boxRect);
