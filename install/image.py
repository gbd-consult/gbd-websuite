...
WKHTMLTOPDF_RELEASE = f'0.12.6.1-2'
WKHTMLTOPDF_PACKAGE = f'wkhtmltox_{WKHTMLTOPDF_RELEASE}.jammy_<ARCH>.deb'
WKHTMLTOPDF_URL = f'https://github.com/wkhtmltopdf/packaging/releases/download/{WKHTMLTOPDF_RELEASE}/{WKHTMLTOPDF_PACKAGE}'
...
class Base:
    ...
    def __init__(self, cmd, args):
        ...
        self.os_image_name = f'ubuntu:{UBUNTU_VERSION}'
        ...
        self.app_image_name = args.get('name') or f'{VENDOR}/gws-{self.arch}:{self.app_version}'
        ...
        self.wkhtmltopdf_package = WKHTMLTOPDF_PACKAGE.replace('<ARCH>', self.arch)
        self.wkhtmltopdf_url = WKHTMLTOPDF_URL.replace('<ARCH>', self.arch)

        # mapserver package built by /install/mapserver/make.sh
        self.mapserver_package = f'/opt/gbd/gbd-mapserver/gbd-mapserver-{MAPSERVER_VERSION}-{self.arch}.tar.gz'
...
class Builder(Base):
    ...
    def prepare(self):
        ...
        # our compiled mapserver, see /install/mapserver
        if not os.path.isdir('gbd-mapserver'):
            if not os.path.isfile(self.mapserver_package):
                cli.run(f'bash {self.this_dir}/mapserver/make.sh all {MAPSERVER_VERSION} {self.arch} {self.mapserver_package}')
            cli.run(f'tar -xzf {self.mapserver_package}')
    ...
    def dockerfile(self):
        ...
        __(f'LABEL Description="{DESCRIPTION}" Vendor="{VENDOR}" Version="{self.app_version}"')
...
class Scanner(Base):
    ...
    def __init__(self, cmd, args):
        ...
        self.trivy_dir = args.get('trivydir') or os.getenv('GWS_TRIVY_DIR') or ''
