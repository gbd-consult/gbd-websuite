...
export class Layer implements types.ILayer {
    ...
    constructor(map: types.IMapManager, props: gws.base.layer.Props) {
        this.map = map;
        this.props = props;
...
export class FeatureLayer extends OlBackedLayer<ol.layer.Vector> implements types.IFeatureLayer {
    ...
    get printPlane(): gws.PrintPlane {
        ...
        return {
            type: this.props.url ? gws.PrintPlaneType.vector : gws.PrintPlaneType.features,
            opacity: this.computedOpacity,
    ...
    beforeDraw() {
        ...
        if (this.loadingStrategy === gws.FeatureLoadingStrategy.bbox) {
        ...
        if (this.loadingStrategy === gws.FeatureLoadingStrategy.all) {
