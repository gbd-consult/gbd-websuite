...
interface DocumentItem {
    ...
    source: {
        type: string;
        url?: string
        params?: api.AssetGetPathParams | api.AssetGetFileParams
    }
}
...
export class Document extends React.PureComponent<DocumentProps> {
    previewUrl() {
        ...
        if (MIME_WITH_PREVIEW.includes(doc.mime)) {
            if (src.type === 'url')
                return src.url;

            if (src.type === 'path') {
                let p = {...src.params, mode: 'preview', r: Math.random()}
                return '/_/cmd/assetHttpGetPath/' + tools.paramsToPath(p);
            }

            if (src.type === 'file') {
                let p = {...src.params, mode: 'preview', r: Math.random()}
                return '/_/cmd/assetHttpGetFile/' + tools.paramsToPath(p);
            }
        }
    }
    ...
    render() {
        ...
        let content = null;
        let label = doc.label || doc.fileName || '';

        if (url) {
            content = <img src={url} alt={label}/>;
        }
