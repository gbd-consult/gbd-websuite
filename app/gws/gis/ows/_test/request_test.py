...
def test_request_ok():
    ...
    gws.gis.ows.request.get(test.web_server_url('ok'), gws.OwsProtocol.WMS, gws.OwsVerb.GetCapabilities)
...
def test_request_failed_with_http_status():
    with test.raises(gws.gis.ows.error.Error):
        gws.gis.ows.request.get(test.web_server_url('NOT_FOUND'), gws.OwsProtocol.WMS, gws.OwsVerb.GetCapabilities)


def test_request_failed_with_content_error_message():
    test.web_server_poke('bad', {'text': '<ServiceException>ERROR</ServiceException>'})
    with test.raises(gws.gis.ows.error.Error):
        gws.gis.ows.request.get(test.web_server_url('bad'), gws.OwsProtocol.WMS, gws.OwsVerb.GetCapabilities)
