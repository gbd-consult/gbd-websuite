...
export class Application implements types.IApplication {
    ...
    async init() {
        ...
        let showLayers = this.options['showLayers'] || [];
        for (let uid of showLayers) {
            let la = this.map.getLayer(uid);
            if (la) {
                this.map.setLayerChecked(la, true);
            }
        }

        let hideLayers = this.options['hideLayers'] || [];
        for (let uid of hideLayers) {
            let la = this.map.getLayer(uid);
            if (la) {
                this.map.setLayerChecked(la, false);
            }
        }
