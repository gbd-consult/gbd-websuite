...
def get_supported_bounds(el, extra_crsids=None) -> t.List[gws.Bounds]:
    ...
    for e in el.all('BoundingBox'):
        crs = gws.gis.crs.get(e.attr('srs') or e.attr('crs'))
        bbox = _bbox_value(e)
        if crs and bbox:
    ...
    def add(new_crs):
        ...
        best = gws.gis.util.best_bounds(new_crs, list(crs_to_b.values()), prefer_projected=False)
        gws.log.debug(f'using {best.crs.srid!r} for missing bounds {new_crs.srid!r}')
        ext = gws.gis.extent.transform(best.extent, best.crs, new_crs)
        crs_to_b[new_crs] = gws.Bounds(crs=new_crs, extent=ext)

    if extra_crsids:
        for c in extra_crsids:
            crs = gws.gis.crs.get(c)
            if crs:
                add(crs)
