...
interface PositionProps extends gc.types.ViewProps {
    mapPointerX: number;
    mapPointerY: number;
}

class PositionView extends gc.View<PositionProps> {
    render() {
...
class PositionWidget extends gc.Controller {
    get defaultView() {
...
interface LoaderProps extends gc.types.ViewProps {
...
class LoaderView extends gc.View<LoaderProps> {
    render() {
        let rc = this.props.appRequestCount;
        let bars = Math.min(rc, LOADER_PROGRESS_MAX);

        return <div {...gc.lib.cls('modInfobarWidget', 'modInfobarLoader', rc && 'isActive')}>
            <Row>
                <Cell>
                    <gc.ui.Button className='modInfobarLoaderIcon'/>
                </Cell>
                {rc > LOADER_PROGRESS_MIN && <Cell width={bars * 2}>
                    {gc.lib.range(bars).map(n => <div className='modInfobarLoaderBar' key={n}/>)}
                </Cell>}
            </Row>
        </div>
    }
...
class LoaderWidget extends gc.Controller {
    get defaultView() {
        return this.createElement(
...
interface ScaleProps extends gc.types.ViewProps {
    controller: ScaleWidget;
    mapEditScale: string;
}

class ScaleView extends gc.View<ScaleProps> {
    render() {
        let cc = this.props.controller;

        let res = this.props.controller.map.resolutions,
            max = Math.max(...res.map(gc.lib.res2scale)),
            min = Math.min(...res.map(gc.lib.res2scale));
        ...
        return <div className="modInfobarWidget modInfobarScale">
            ...
            <Cell className="modInfobarScaleSlider">
                <gc.ui.Slider
                    minValue={min}
                    maxValue={max}
                    value={gc.lib.asNumber(this.props.mapEditScale)}
...
class ScaleWidget extends gc.Controller {
    async init() {
...
interface RotationProps extends gc.types.ViewProps {
    controller: RotationWidget;
    mapEditAngle: number;
}

class RotationView extends gc.View<RotationProps> {
    update(value) {
    ...
    render() {
        let cc = this.props.controller;
        let n = gc.lib.asNumber(this.props.mapEditAngle);
...
class RotationWidget extends gc.Controller {
    async init() {
...
class Spacer extends gc.Controller {
...
interface LinkProps extends gc.types.ViewProps {
...
class LinkView extends gc.View<LinkProps> {
    render() {
        return <div {...gc.lib.cls('modInfobarWidget', 'modInfobarLink', this.props.className)}>
            <a
...
class LinkButtonView extends gc.View<LinkProps> {
    render() {
        return <gc.ui.Button
...
class LinkWidget extends gc.Controller {
...
interface AboutViewProps extends gc.types.ViewProps {
...
class AboutDialog extends gc.View<AboutViewProps> {
    render() {
        let cc = this.props.controller;
        ...
        return <gc.ui.Dialog
            ...
            buttons={[ok]}
        >{content}</gc.ui.Dialog>
    }
}


class AboutWidget extends gc.Controller {
...
class InfobarController extends gc.Controller {
