import * as React from 'react';
import * as util from './util';


interface TrackerProps {
    xValueMin?: number;
    xValueMax?: number;
    yValueMin?: number;
    yValueMax?: number;

    xValue?: number;
    yValue?: number;

    noHandle?: boolean;
    noMove?: boolean;
    idleInterval?: number;

    whenPressed?: () => void;
    whenReleased?: () => void;
    whenChanged: (x: number, y: number) => void;
}

export class Tracker extends React.PureComponent<TrackerProps> {
    rootRef: React.RefObject<any>;
    handleRef: React.RefObject<any>;
    handlers: any;

    xMin: number;
    xMax: number;
    yMin: number;
    yMax: number;

    dx: number;
    dy: number;

    idleTimer: any;

    constructor(props) {
        super(props);

        this.rootRef = React.createRef();
        this.handleRef = React.createRef();

        this.handlers = {
            mousemove: this.onMouseMove.bind(this),
            mouseup: this.onMouseUp.bind(this),
            touchmove: this.onTouchMove.bind(this),
            touchend: this.onTouchEnd.bind(this),
            touchcancel: this.onTouchCancel.bind(this)
        }
    }

    render() {
        return <button
            className="uiTracker"
            tabIndex={-1}
            ref={this.rootRef}
            onMouseDown={e => this.onMouseDown(e)}
            onTouchStart={e => this.onTouchStart(e)}
        >
            {!this.props.noHandle && <div className="uiTrackerHandle" ref={this.handleRef}/>}
        </button>;
    }

    componentDidMount() {
        let r = this.rootRef.current,
            h = this.handleRef.current,
            xmin = this.props.xValueMin || 0,
            xmax = this.props.xValueMax || 0,
            ymin = this.props.yValueMin || 0,
            ymax = this.props.yValueMax || 0;


        if (xmin === xmax) {
            this.xMin = this.xMax = r.offsetWidth >> 1;
        } else if (h) {
            this.xMin = h.offsetWidth >> 1;
            this.xMax = r.offsetWidth - this.xMin;
        } else {
            this.xMin = 0;
            this.xMax = r.offsetWidth;
        }

        if (ymin === ymax) {
            this.yMin = this.yMax = r.offsetHeight >> 1;
        } else if (h) {
            this.yMin = h.offsetHeight >> 1;
            this.yMax = r.offsetHeight - this.yMin;
        } else {
            this.yMin = 0;
            this.yMax = r.offsetHeight;
        }

        this.dx = h ? (h.offsetWidth >> 1) : 0;
        this.dy = h ? (h.offsetHeight >> 1) : 0;

        this.updatePositionFromValues(this.props.xValue || 0, this.props.yValue || 0);
    }

    componentDidUpdate() {
        this.updatePositionFromValues(this.props.xValue || 0, this.props.yValue || 0);
    }

    componentWillUnmount() {
        this.trackStop();
    }

    //

    protected onTouchStart(e: React.TouchEvent<any>) {
        if (e.touches.length === 1) {
            this.trackStart(e.touches[0].clientX, e.touches[0].clientY);
            e.preventDefault();
            e.stopPropagation();
        }
    }

    protected onMouseDown(e: React.MouseEvent<any>) {
        this.trackStart(e.clientX, e.clientY);
        e.preventDefault();
        e.stopPropagation();
    }

    protected onMouseMove(e: MouseEvent) {
        this.trackUpdate(e.clientX, e.clientY);
        e.preventDefault();
    }

    protected onMouseUp(e: MouseEvent) {
        this.trackStop();
        e.preventDefault();
    }

    protected onTouchMove(e: TouchEvent) {
        if (e.touches.length === 1) {
            this.trackUpdate(e.touches[0].clientX, e.touches[0].clientY);
            e.preventDefault();
        }
    }

    protected onTouchEnd(e: TouchEvent) {
        this.trackStop();
        e.preventDefault();
    }

    protected onTouchCancel(e: TouchEvent) {
        this.trackStop();
        e.preventDefault();
    }

    //

    protected trackStart(x, y) {
        if (this.rootRef.current)
            this.rootRef.current.focus();

        if (this.props.whenPressed)
            this.props.whenPressed();

        this.trackUpdate(x, y);

        let doc = this.rootRef.current.ownerDocument;
        Object.keys(this.handlers).forEach(e => doc.addEventListener(e, this.handlers[e]));
    }

    protected trackStop() {
        clearTimeout(this.idleTimer);

        let doc = this.rootRef.current.ownerDocument;
        Object.keys(this.handlers).forEach(e => doc.removeEventListener(e, this.handlers[e]));

        if (this.props.whenReleased)
            this.props.whenReleased();
    }

    protected trackUpdate(x, y) {
        clearTimeout(this.idleTimer);

        let b = this.rootRef.current,
            r = b.getBoundingClientRect(),
            px = util.constrain(x - r.left, this.xMin, this.xMax),
            py = util.constrain(y - r.top, this.yMin, this.yMax),
            vx = util.translate(px, this.xMin, this.xMax, this.props.xValueMin || 0, this.props.xValueMax || 0),
            vy = util.translate(py, this.yMin, this.yMax, this.props.yValueMin || 0, this.props.yValueMax || 0);

        //this.setPosition(px, py);

        if (!this.props.noMove)
            this.props.whenChanged(vx, vy);

        this.trackIdle(vx, vy);
    }

    protected trackIdle(vx, vy) {
        clearTimeout(this.idleTimer);

        if (this.props.idleInterval)
            this.idleTimer = setTimeout(() => this.callIdle(vx, vy), this.props.idleInterval);
    }

    protected callIdle(vx, vy) {
        this.props.whenChanged(vx, vy);
        this.trackIdle(vx, vy);
    }

    protected updatePositionFromValues(xValue, yValue) {
        this.setPosition(
            util.translate(xValue, this.props.xValueMin || 0, this.props.xValueMax || 0, this.xMin, this.xMax),
            util.translate(yValue, this.props.yValueMin || 0, this.props.yValueMax || 0, this.yMin, this.yMax));
    }

    protected setPosition(x, y) {
        let h = this.handleRef.current;

        if (h) {
            h.style.left = (x - this.dx) + 'px';
            h.style.top = (y - this.dy) + 'px';
        }
    }
}

