...
class _Runtime(chartreux.Runtime):
    ...
    def _render_map(self, kwargs):
        ...
        mro = gws.lib.render.render_map(mri, self.notify)
        return gws.lib.render.output_to_html_string(mro)
    ...
    def _render_legend(self, kwargs):
        ...
        lro = gws.lib.legend.render(gws.Legend(layers=layers))
        if not lro:
        ...
        img_path = gws.lib.legend.to_image_path(lro, self.tri.out_path + '_legend_' + str(self.legend_count))
        if not img_path:
