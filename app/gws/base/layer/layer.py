...
class Config(t.WithTypeAndAccess):
    ...
    meta: t.Optional[gws.base.metadata.Config]  #: layer meta data
...
class Layer(gws.Object, t.ILayer):
    ...
    def configure(self):
        ...
        self.map: t.IMap = t.cast(t.IMap, self.get_closest('gws.base.map'))
        ...
        self.description_template: t.ITemplate = gws.base.template.find(self.templates, subject='layer.description')

        p = self.var('dataModel')
        self.data_model: t.Optional[t.IModel] = self.create_child('gws.base.model', p) if p else None
        ...
        self.edit_data_model: t.Optional[t.IModel] = self.create_child('gws.base.model', p) if p else None
        self.edit_options: t.Data = self.var('edit')

        p = self.var('editStyle')
        self.edit_style: t.Optional[t.IStyle] = gws.base.style.from_config(p) if p else None
    ...
    def configure_metadata(self, provider_meta=None) -> t.MetaData:
        ...
        return gws.base.metadata.from_config(m)
