...
def start(req: t.IRequest, p: pt.PrintParams) -> gws.lib.job.Job:
    job = _create(req, p)
...
def _create(req: t.IRequest, p: pt.PrintParams) -> gws.lib.job.Job:
    cleanup()
...
class _Worker:
    def __init__(self, root: t.IRootObject, job: gws.lib.job.Job, base_dir, p: pt.PrintParams, user: t.IUser):
        self.root = root
        self.job = job
        ...
        self.project = t.cast(t.IProject, self.acquire('gws.base.project', job.project_uid))
    ...
    def prepare_render_item(self, item: pt.PrintItem):
        ...
        if item.type == 'fragment':
            ...
            ii.fragment = t.SvgFragment(
                ...
                tags=item.tags or [],
                styles=[gws.base.style.from_props(s) for s in (item.styles or [])])
            ii.dpi = units.PDF_DPI
            return ii
