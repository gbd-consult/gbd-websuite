...
abstract class BaseVectorLayer extends OlBackedLayer<ol.layer.Vector> implements types.IMapFeatureLayer {
    ...
    constructor(map, props) {
        ...
        if (props.style) {
            this.styleName = this.map.style.nameFor(props.style);
            console.log(props.style, this.styleName)


        }

        if (props.editStyle)
            this.editStyleName = this.map.style.nameFor(props.editStyle);
    ...
    createOLayer() {
        return new ol.layer.Vector({
            ...
            style: this.map.style.functionFor(this.styleName),
        });
    }

    setStyle(styleName) {
        this.styleName = styleName;
        this.oLayer.setStyle(this.map.style.functionFor(this.styleName));
    }
...
export class FeatureLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    get printItem(): api.PrintItem {
        let fs = tools.compact(this.features.map(f => f.props));
        ...
        return {
            ...
            printAsVector: true,
            features: fs,
            style: this.map.style.propsFor(this.styleName),
        };
    }
...
export class VectorLayer extends BaseVectorLayer implements types.IMapFeatureLayer {
    ...
    addFeatures(fs) {
        ...
        this.source.addFeatures(toAdd.map(f => f.oFeature));
