...
def _run2(action, src_dir, replace, au_uid, job):
    ...
    for p in sorted(shp_paths):
        ...
        with gws.lib.gdal2.from_path(p) as ds:
            for f in gws.lib.gdal2.features(ds, action.crs, encoding=_encoding(p)):
                r = {}
                ...
                if not f.shape:
                    ...
                    try:
                        f.shape = gws.lib.shape.from_geometry({
                            "type": "Point",
                            "coordinates": [
    ...
    table: gws.SqlTable = action.plan_table
    ...
    with action.db.connect() as conn:
        ...
        for rec in rs:
            if rec['_au'] not in au_uids:
                continue
            extents[rec['_au']] = gws.lib.extent.from_box(rec['p'])
...
def _geom_name(s: gws.IShape):
    if s.type == gws.GeometryType.multipoint:
        return '_geom_x'
    if s.type == gws.GeometryType.multilinestring:
        return '_geom_l'
    if s.type == gws.GeometryType.multipolygon:
        return '_geom_p'
