...
# getattr needs to be defined out of class, otherwise the type checker will accept any attribute
...
class ServerMonitor(Node):
    ...
    def register_periodic_task(self, obj: Node, frequency: int = 0):
        """Register an object as a periodic task handler.

        Args:
            obj: A node with a ``periodic_task`` method.
            frequency: Task frequency in seconds.
        """
...
class AuthSessionManager(Node):
    ...
    maxLifeTime: int
    """Absolute session lifetime in seconds, or 0 if not limited."""
    ...
    def get(self, uid: str) -> Optional['AuthSession']:
        ...
        Returns:
            A Session or ``None`` if uid does not exist or the Session is not valid.
        """

    def list_all(self) -> list['AuthSession']:
        """Get all sessions."""
...
class Model(Node):
    ...
    def get_feature(self, uid: str | int, mc: ModelContext) -> Optional['Feature']: ...
