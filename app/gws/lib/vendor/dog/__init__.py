...
Given a set of root filsystem folders, the Generator collects all matching files (eg `doc/*.md`), combines them together
...
The DOG documentation is a nested hierachy of section.

A *section* starts with a Markdown header, optionally follwed by a colon and a sid.

    # Introduction :/docs/developer/intro

Deeper headings (with more dashes) create a hierachy.
...
Heading levels are only used to create hierachies, the ouput rendering (h1, h2 etc) is determined by
...
The topmost section in a file must provide an sid and this sid has to be absolute.
...
Relative sids are resolved relatively to the contaning section sid.
