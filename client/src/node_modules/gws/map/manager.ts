...
export class MapManager implements types.IMapManager {
    ...
    setExtraInteractions(ixs) {
        this.setInteractions(this.defaultInteractions.concat(ixs));
    }
    ...
    protected initEvents() {
        ...
        this.oView.on('change:resolution', () => this.viewChanged());
        this.oView.on('change:center', () => this.viewChanged());
        this.oView.on('change:rotation', () => this.viewChanged());
    }
    ...
    getStyle(p) {
        if (p instanceof CssStyle)
            return p;
        if (p.type === 'css')
            return CssStyle.fromCss(this, p.content || p.text);
        if (p.type === 'cssSelector')
            return CssStyle.fromSelector(this, p.text);
    ...
    styleFunction(style?: types.IMapStyle) {
        if (!style)
            return null;
        return function (oFeature, resolution) {
    ...
    bitmapAsDataURL(boxRect) {
        let canvas = this.oMap.getViewport().firstChild as HTMLCanvasElement;
    ...
    async printParams(boxRect, dpi) {
        ...
        if (useBitmaps) {
            ...
            bitmap = await tools.delay(200, () => this.bitmapAsDataURL(boxRect));
    ...
    protected walk(layer, fn: (la: types.IMapLayer) => any) {
