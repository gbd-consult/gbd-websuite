...
def parse(gen: base.Generator, parse_all=False):
    ...
    for chunk in gen.chunks:
        for path in chunk.paths.get('python', []):
            _parse_path(gen, path, chunk.name, chunk.sourceDir, parse_all)


def _init_parser(gen: base.Generator):
    for b in base.v.BUILTINS:
...
def _parse_path(gen: base.Generator, path: str, base_name: str, base_dir: str, parse_all=True):
    ...
    try:
        ...
        raise base.GeneratorError(f'parse error: {msg} in {path}:{lineno}')
...
class _PythonParser:
    ...
    def run(self):
        ...
        for node in ast.walk(tree):
            if _cls(node) == 'Module':
                self.moduleNode = cast(ast.Module, node)
                break
        else:
        ...
        typ = self.add(
            ...
            doc=self.inner_doc(self.moduleNode),
        )
        self.tModule = typ.uid

        self.imports = self.prepare_imports()

        for node in self.nodes(self.moduleNode.body):
            cc = _cls(node)
            if cc == 'Expr':
    ...
    def prepare_imports(self):
        ...
        for node in self.nodes(self.moduleNode.body, 'Import'):
            for nn in node.names:
                imp[nn.asname or nn.name] = nn.name

        for node in self.nodes(self.moduleNode.body, 'ImportFrom'):
        ...
        for alias, target in imp.items():
            if _is_type_name(alias) and not _builtin_name(alias):
                self.gen.aliases[self.moduleName + DOT + alias] = target
    ...
    def parse_ext_declaration(self, node):
        ...
        try:
            ...
            return
        if not decl.startswith(base.v.EXT_DECL_PREFIX):
            return
        if not call.args:
        ...
        self.add(c=base.c.EXT, extName=base.v.EXT_OBJECT_PREFIX + tail, tTarget=self.qname(args.pop(0) if args else base.v.EXT_OBJECT_CLASS))
        self.add(c=base.c.EXT, extName=base.v.EXT_CONFIG_PREFIX + tail, tTarget=self.qname(args.pop(0) if args else base.v.EXT_CONFIG_CLASS))
    ...
    def parse_property(self, owner_typ: base.Type, node, doc: str, annotated: bool):
        ...
        if c == base.c.LITERAL:
            ...
            typ.hasDefault = True
        if c == base.c.EXPR:
            # evaluated later on in normalizer._evaluate_defaults
            typ.defaultExpression = [c, value]
    ...
    def gws_decorator(self, node, kind):
        for d in getattr(node, 'decorator_list', []):
            ...
            name = _name(d.func)
            if not name.startswith(base.v.EXT_PREFIX):
                continue
    ...
    def type_from_node(self, node) -> base.Type:
        ...
        if cc == 'Subscript':
            ...
            return self.type_from_name(self.qname(node.value), node.slice.value if _cls(node.slice) == 'Index' else node.slice)

        # foo: [SomeType, SomeType]
        if cc in {'List', 'Tuple'}:
            item_types = [self.type_from_node(e) for e in node.elts]
            return self.add(c=base.c.TUPLE, tItems=[typ.uid for typ in item_types])
        ...
        if cc == 'BinOp' and _cls(node.op) == 'BitOr':
            ...
            return self.add(c=base.c.UNION, tItems=[typ.uid for typ in item_types])
    ...
    def type_from_name(self, name: str, param=None) -> base.Type:
        ...
        if g == 'Literal':
            ...
            return self.add(c=base.c.LITERAL, literalValues=vals)
        ...
        if g == 'Optional':
            ...
            return self.add(c=base.c.OPTIONAL, tTarget=param_typ.uid)

        if g.lower() == 'list':
            return self.add(c=base.c.LIST, tItem=param_typ.uid if param_typ else 'any')

        if g.lower() == 'iterable':
            return self.add(c=base.c.LIST, tItem=param_typ.uid if param_typ else 'any')

        if g.lower() == 'set':
            return self.add(c=base.c.SET, tItem=param_typ.uid if param_typ else 'any')
        ...
        if g.lower() == 'dict':
            if param_items:
                ...
                key = 'str'
                val = 'any'
            return self.add(c=base.c.DICT, tKey=key, tValue=val)

        if g == 'Union':
            if not param_items:
                raise ValueError('invalid Union')
            return self.add(c=base.c.UNION, tItems=sorted(param_items))

        if g.lower() == 'tuple':
            if not param_typ:
                return self.add(c=base.c.TUPLE, tItems=[])
            if not param_items:
                raise ValueError('invalid Tuple')
            return self.add(c=base.c.TUPLE, tItems=list(param_items))

        if g.lower() == 'callable':
            if not param_typ:
                return self.add(c=base.c.CALLABLE, tItems=[])
            if not param_items:
                raise ValueError('invalid Callable')
            return self.add(c=base.c.CALLABLE, tItems=list(param_items))
        ...
        if g:
            ...
            return self.add(c=base.c.ATOM, name=name)

        return self.add(c=base.c.UNDEFINED, name=name)
...
def _builtin_name(name: str) -> str:
    if name in base.v.BUILTINS:
        return name
    if name in base.v.BUILTIN_TYPES:
        return name
    for b in base.v.BUILTIN_TYPES:
