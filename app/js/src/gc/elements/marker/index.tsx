...
class MarkerLayer extends gc.map.layer.FeatureLayer {
...
class MarkerController extends gc.Controller {
    layer: MarkerLayer;
        ...
        if (desc.length > 0) {
            this.update({
                infoboxContent: <components.Infobox controller={this}>
                    <gc.ui.HtmlBlock content={desc.join("\n")}/>
                </components.Infobox>
            })
        }
    }
    ...
    show(content) {
        ...
        let features = content.features,
            geoms = gc.lib.compact(features.map(f => f.geometry));
        ...
        if (mode.fade) {
            gc.lib.debounce(() => this.reset(), FADE_DURATION)();
        }
